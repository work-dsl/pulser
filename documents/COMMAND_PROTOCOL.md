...
**数据载荷：** 状态数据（格式：`模式(1字节) | 状态(1字节) | 次数(2字节，小端序)`）
...
- `状态` (1字节):
  - `0x00`: PULSE_STATUS_IDLE（空闲状态）
  - `0x01`: PULSE_STATUS_RUNNING（运行状态）
  - `0x02`: PULSE_STATUS_ERROR（错误状态）
  - `0x03`: 完成状态（空闲状态且计数>0，表示已完成）
- `次数低` (1字节): 输出次数低字节
- `次数高` (1字节): 输出次数高字节（小端序）
...
上报状态（正常模式，完成，输出5次）：
```
发送: FA 0E 00 00 03 3A 02 
      00 03 05 00
      // 长度=14(0x0E), 模式=0x00(正常), 状态=0x03(完成), 次数=5 (小端)
      [CRC16低] [CRC16高] 0D
```

---

### 4.12 过流状态信息上报 (CMD_OVER_CURRENT_STATUS_UPLOAD = 0x3B)

**功能说明：** 从机主动上报过流状态信息（非应答型命令）。

**发送帧（从机→主机）：**

```
FA [长度] 00 03 3B 02 [状态数据] [CRC16低] [CRC16高] 0D
```

**数据载荷：** 状态数据（格式：`通道(1字节) | 过流电压值(2字节，小端序)`）

**数据格式：**
- `通道` (1字节):
  - `0x00`: 通道1（ADC1/COMP3，使用DAC1）
  - `0x01`: 通道2（ADC2/COMP1，使用DAC3）
- `过流电压值低` (1字节): 过流电压值低字节（单位：mV）
- `过流电压值高` (1字节): 过流电压值高字节（小端序）
...
上报过流状态（通道1，过流电压1650mV）：
```
发送: FA 0B 00 00 03 3B 02 
      00 72 06
      // 长度=11(0x0B), 通道=0x00(通道1), 电压=1650=0x0672 (小端)
      [CRC16低] [CRC16高] 0D
```

---

### 4.13 读取硬件IO过流保护引脚状态 (CMD_GET_OCP_PIN_STATUS = 0x3C)

**功能说明：** 查询硬件IO过流保护引脚的状态。

**发送帧（主机→从机）：**

```
FA 09 00 03 3C 02 [CRC16低] [CRC16高] 0D
```
...
```
FA 0D 00 03 3C 02 00 [安全状态] [正脉冲引脚状态] [负脉冲引脚状态] [CRC16低] [CRC16高] 0D
```
...
- `[安全状态]` (1字节):
  - `0x00`: 安全（两个引脚都为低）
  - `0x01`: 不安全（至少一个引脚为高）
- `[正脉冲引脚状态]` (1字节): 正脉冲引脚状态（0=低，1=高）
- `[负脉冲引脚状态]` (1字节): 负脉冲引脚状态（0=低，1=高）
...
查询引脚状态（安全，正脉冲引脚=0，负脉冲引脚=0）：
```
接收: FA 0D 00 03 3C 02 00 00 00 00 [CRC16低] [CRC16高] 0D
```

---

### 4.14 复位硬件过流保护 (CMD_RESET_OCP_HARDWARE = 0x3D)

**功能说明：** 复位硬件过流保护引脚（将复位引脚置高）。

**发送帧（主机→从机）：**

```
FA 09 00 03 3D 02 [CRC16低] [CRC16高] 0D
```
...
```
FA 0B 00 03 3D 02 00 [安全状态] [CRC16低] [CRC16高] 0D
```
...
**应答说明：**
- 成功时返回ACK_OK和复位后的安全状态
- 参数长度不正确时返回ACK_ERR_INVALID_PARAM
- 操作异常时返回ACK_ERR_OPERATE_ABNORMAL
- 如果复位后状态为安全（0x00），系统会自动解除脉冲锁定

**示例：**

复位硬件过流保护（复位后状态为安全）：
```
接收: FA 0B 00 03 3D 02 00 00 [CRC16低] [CRC16高] 0D
```
...
| 0x3A | 脉冲序列输出完成状态上报 | 从机→主机 | 4 | 无 |
| 0x3B | 过流状态信息上报 | 从机→主机 | 3 | 无 |
| 0x3C | 读取硬件IO过流保护引脚状态 | 主机→从机 | 0 | 4 (ACK+状态) |
| 0x3D | 复位硬件过流保护 | 主机→从机 | 0 | 2 (ACK+状态) |
