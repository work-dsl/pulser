...
- 将复位引脚置高，用于硬件恢复
- 复位后需要检查引脚状态确认是否真正安全
- 建议复位后调用`current_monitor_get_ocp_pin_status()`确认安全状态

**使用示例**：
```c
/* 执行硬件复位 */
if (current_monitor_reset_ocp_hardware() == 0) {
    /* 检查是否真正安全 */
    if (current_monitor_get_ocp_pin_status() == 0U) {
...
HAL_COMP_TriggerCallback()
    ↓
current_monitor_handle_ocp_trigger()
    ↓
